...
type Width = "auto" | "full" | "card";

export function WhatsAppButton({ shape = "square", width = "auto", className = "" }: { shape?: Shape; width?: Width; className?: string }) {
  const radius = shape === "pill" ? "rounded-full" : "rounded-none";
  const w = width === "full" ? "w-full" : width === "card" ? "w-full max-w-[311px]" : "";
  return (
    <a
      href={WHATSAPP_URL}
      target="_blank"
...
export function PhoneButton({ shape = "square", width = "auto", className = "" }: { shape?: Shape; width?: Width; className?: string }) {
  const radius = shape === "pill" ? "rounded-full" : "rounded-none";
  const w = width === "full" ? "w-full" : width === "card" ? "w-full max-w-[311px]" : "";
  return (
    <a
...
export default function CTAButtons({
  ...
  return (
    <div className={`flex flex-col sm:flex-row gap-3 justify-center items-stretch ${className}`}>
      <WhatsAppButton shape={shape} width={width} />
      <PhoneButton shape={shape} width={width} />
    </div>
  );
}
